import { describe, expect, test } from "bun:test";
import { powerlineTheme } from "./powerlineTheme";

const RPST = "\uE0B0";

describe("powerlineTheme", () => {
  const baseStatus = {
    session_id: "3694cfa9-53b4-4025-ab41-f8fa913a189b",
    transcript_path: "/home/testuser/.claude/projects/test.jsonl",
    cwd: "/home/testuser/Projects/statusline",
    model: { id: "claude-opus-4-5", display_name: "Opus 4.5" },
    workspace: {
      current_dir: "/home/testuser/Projects/statusline",
      project_dir: "/home/testuser/Projects/statusline",
    },
    version: "2.1.3",
    output_style: { name: "Explanatory" },
    cost: {
      total_cost_usd: 0,
      total_duration_ms: 1345,
      total_api_duration_ms: 0,
      total_lines_added: 0,
      total_lines_removed: 0,
    },
    context_window: {
      total_input_tokens: 0,
      total_output_tokens: 0,
      context_window_size: 200000,
      current_usage: null,
      used_percentage: null,
      remaining_percentage: null,
      vim: { mode: "NORMAL" },
      agent: { name: "main", type: "primary" },
    },
    exceeds_200k_tokens: false,
  };

  describe("when input is empty or undefined", () => {
    test("returns empty string for undefined input", async () => {
      const result = await powerlineTheme(undefined);
      expect(result).toBe("");
    });

    test("returns empty string for empty string input", async () => {
      const result = await powerlineTheme("");
      expect(result).toBe("");
    });
  });

  describe("when input is invalid JSON or schema", () => {
    test("returns empty string for invalid JSON", async () => {
      const result = await powerlineTheme("not valid json");
      expect(result).toBe("");
    });

    test("returns empty string for valid JSON but invalid schema", async () => {
      const result = await powerlineTheme('{"foo": "bar"}');
      expect(result).toBe("");
    });

    test("returns empty string for null input", async () => {
      const result = await powerlineTheme("null");
      expect(result).toBe("");
    });
  });

  describe("when input is valid status JSON", () => {
    test("renders non-empty output", async () => {
      const result = await powerlineTheme(JSON.stringify(baseStatus));
      expect(result.length).toBeGreaterThan(0);
    });

    test("contains model info with version", async () => {
      const result = await powerlineTheme(JSON.stringify(baseStatus));
      expect(result).toContain("🤖");
      expect(result).toContain("opus-4-5");
      expect(result).toContain("2.1.3");
    });

    test("contains session ID", async () => {
      const result = await powerlineTheme(JSON.stringify(baseStatus));
      expect(result).toContain("📃");
      expect(result).toContain("3694cfa9-53b4-4025-ab41-f8fa913a189b");
    });

    test("contains project directory", async () => {
      const result = await powerlineTheme(JSON.stringify(baseStatus));
      expect(result).toContain("🗂️");
      expect(result).toContain("statusline");
    });

    test("contains git status", async () => {
      const result = await powerlineTheme(JSON.stringify(baseStatus));
      const hasGitStatus =
        result.includes("🌿") ||
        result.includes("🪾") ||
        result.includes("💾") ||
        result.includes("💥");
      expect(hasGitStatus).toBe(true);
    });

    test("contains powerline separator characters", async () => {
      const result = await powerlineTheme(JSON.stringify(baseStatus));
      expect(result).toContain(RPST);
    });
  });

  describe("usage percentage rendering", () => {
    const createStatusWithUsage = (usedPercentage: number | null) => ({
      ...baseStatus,
      context_window: {
        ...baseStatus.context_window,
        total_input_tokens: 100,
        total_output_tokens: 100,
        used_percentage: usedPercentage,
        remaining_percentage: usedPercentage ? 100 - usedPercentage : null,
      },
    });

    test("does not render usage segment when usage is 0%", async () => {
      const status = createStatusWithUsage(0);
      const result = await powerlineTheme(JSON.stringify(status));
      expect(result).not.toContain("0%");
    });

    test("does not render usage segment when usage is null", async () => {
      const status = createStatusWithUsage(null);
      const result = await powerlineTheme(JSON.stringify(status));
      expect(result).not.toContain("%");
    });

    test("renders usage segment when usage is 42%", async () => {
      const status = createStatusWithUsage(42);
      const result = await powerlineTheme(JSON.stringify(status));
      expect(result).toContain("42%");
    });

    test("renders usage segment when usage is 100%", async () => {
      const status = createStatusWithUsage(100);
      const result = await powerlineTheme(JSON.stringify(status));
      expect(result).toContain("100%");
    });
  });

  describe("line wrapping", () => {
    test("renders on single line when segments fit within terminal width", async () => {
      const status = { ...baseStatus, session_id: "abc" };
      const result = await powerlineTheme(JSON.stringify(status));
      const lines = result.split("\n");
      expect(lines.length).toBe(1);
    });

    test("wraps to multiple lines when segments exceed terminal width", async () => {
      const longSessionId = "a".repeat(100);
      const status = { ...baseStatus, session_id: longSessionId };
      const result = await powerlineTheme(JSON.stringify(status));
      const lines = result.split("\n");
      expect(lines.length).toBeGreaterThan(1);
    });

    test("each wrapped line contains separator characters", async () => {
      const longSessionId = "a".repeat(100);
      const status = { ...baseStatus, session_id: longSessionId };
      const result = await powerlineTheme(JSON.stringify(status));
      const lines = result.split("\n");
      for (const line of lines) {
        expect(line).toContain(RPST);
      }
    });
  });

  describe("with fixture files", () => {
    test("renders with statusline-1.json (null usage)", async () => {
      const fixture = await Bun.file(
        `${import.meta.dir}/../../fixtures/statusline-1.json`,
      ).json();
      const result = await powerlineTheme(JSON.stringify(fixture));
      expect(result.length).toBeGreaterThan(0);
      expect(result).toContain("🤖");
      expect(result).toContain("📃");
      expect(result).toContain("🗂️");
      expect(result).not.toContain("%");
    });

    test("renders with statusline-2.json (with usage data)", async () => {
      const fixture = await Bun.file(
        `${import.meta.dir}/../../fixtures/statusline-2.json`,
      ).json();
      const result = await powerlineTheme(JSON.stringify(fixture));
      expect(result.length).toBeGreaterThan(0);
      expect(result).toContain("0.38%");
    });
  });
});
